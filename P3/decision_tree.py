...
class TreeNode(object):
	...
	def split(self):
		...
		# self.dim_split = 0
		# x = np.array(self.features)[:, self.dim_split]
		# print(x)
			min_entropy = 100000
		...
		if not self.splittable:
			return
		# xcopy = [x for x in self.features]
		# ycopy = [x for x in self.labels]
		...
		for val in self.feature_uniq_split:
			...
			x_child = x[indexes].tolist()
			y_child = np.array(self.labels)[indexes].tolist()
			child = TreeNode(x_child, y_child, self.num_cls)
			if len(x_child) == 0 or x_child[0].count(None) == len(x_child[0]):
