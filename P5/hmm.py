...
def forward(pi, A, B, O):
  ...
  alpha[:, 0] = pi * B[:, O[0]]
  for i in range(1, N):
    alpha[:, i] = B[:, O[i]] * np.dot(A.T, alpha[:, i-1])
  ###################################################
...
def backward(pi, A, B, O):
  ...
  ###################################################
  # Q3.1 Edit here
  beta[:, N-1] = np.asarray([1]*S)
  for i in range(N-1, -1, -1):
    beta[:, i-1] = np.sum(beta[:, i]*A*B[:, O[i]], axis=1)
  ###################################################
...
def viterbi(pi, A, B, O):
  ...
  Z[:, 0] = -1
  for i in range(1, N):
    delta_temp = delta[:, i-1] * A.T
    Z[:, i] = np.max(delta_temp, axis=1)
    delta[:, i] = np.max(delta_temp, axis=1) * B[:, O[i]]

  Z = Z.astype(int)
  states = np.max(delta[:, N-1])
